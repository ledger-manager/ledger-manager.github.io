...
import { StockVal, createEmptyStockVal } from '../models/stock-val.model';
...
export class StockEntryComponent implements OnInit, OnDestroy {
    ...
    loadData(): void {
      ...
      this.productService.getPrice().pipe(take(1)).subscribe(price => {
        ...
        this.stockService.getStock(key).pipe(
          take(1),
          finalize(() => {
          ...
          (payload?.values || []).forEach((v: any) => {
            const savedKey = v?.item_id ?? v?.seq;
            if (savedKey !== undefined && savedKey !== null) {
              savedMap.set(savedKey, v);
            }
          });
  ...
  resetState(): void {
    // Reset local form and flags when date/location changes
  ...
  setupAutoSave(): void {
    // Very small auto-save skeleton: if there are changes, we could automatically save periodically
    // For now keep it a no-op — explicit save by user is preferred for safety.
  }
