...
export class ReceiptService {
  ...
  private buildDocId(key: string): string {
    return `RECEIPT_${this.resolveDateToken(key)}`;
  }

  /**
   * Return receipt payload for a date key.
   */
  getReceipts(dateKey: string): Observable<StockPayload> {
  ...
  saveReceipts(payload: StockPayload): Observable<boolean> {
